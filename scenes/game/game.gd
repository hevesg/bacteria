...
func _on_timer_timeout() -> void:
	...
	for herbivore in herbivore_container.get_children():
		if herbivore is Herbivore:
			total_herbivore_energy += herbivore.energy
			total_herbivore_cumulative_energy += herbivore._cumulative_energy
	
	for algae in algae_container.get_children():
		if algae is Algae:
			total_algae_energy += algae.energy
			total_algae_cumulative_energy += algae._cumulative_energy
			if algae.is_alive():
				algae_alive += 1
			else:
				algae_dead += 1
...
func _on_herbivore_drop(mouse_position: Vector2) -> void:
	...
	herbivore_container.spawn(pos, randf() * PI * 2, Globals.MILLION, Globals.MILLION)
